...
## REQ-014 좌우 분할 영역 크기 조절

좌우로 나뉜 화면 영역(커밋 목록/배포 미리보기, 포함된 파일/누락된 의존성)의 폭 비율을 사용자가 마우스 드래그로 조절할 수 있어야 한다.

조절한 비율은 앱을 재실행해도 유지된다.

각 영역은 최소 폭 아래로는 줄어들지 않으며, 창이 두 영역의 최소 폭 합보다 좁아지면 가로 스크롤이 나타난다.

추가 (실사용 중 나온 요구, RISK_ISSUES.md §7.4, 2026-08-07)
