...
- 바로 다음 세션에서 사용자가 케이스 D("N개 선택됨" 카운터를 추가하지
  ...
  결정 이력 #31 참고.
- v0.2.1 이후, 실사용 중 나온 아이디어 3건이 추가로 구현 완료됐습니다
  (2026-08-12, 아직 새 버전 태그/릴리스는 안 만듦 — package.json은 여전히
  0.2.1): (1) DeployFilesPanel 좌/우 전체선택·전체추가가 필터/검색을
  ...
  검증 완료. 다음에 버전을 올릴 때 이 3건도 릴리스 노트에 포함하세요.
