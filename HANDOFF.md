...
- 실사용 중 나온 개선 아이디어 4건(파일명 커밋 검색, 체크한 커밋 유지, 사용자정의
  Export 경로, 의존성 완결성 검사)+드래그 리사이즈 1건을 이미 요구사항 수준까지
  구체화해뒀습니다 — RISK_ISSUES.md §6/§7에 배경·트레이드오프·확정 설계·TO-BE
  와이어프레임까지 다 있습니다. 이번 작업이 이 중 하나라면 처음부터 다시 요구사항을
  도출하지 말고 거기서부터 시작하세요(§0.2와 같은 이유 — 이미 있는 걸 임의로
  다시 만들지 않는다). REQUIREDMENT.md/DETAILED_DESIGN.md 등 정식 문서로의 편입은
  아직 안 돼 있으니, 구현하면서 편입하면 됩니다.
- 이 5건의 권장 구현 순서(근거는 RISK_ISSUES.md §7 도입부 참고): 1) §7.1(Export
  경로) 단독 → 2) §7.2(의존성 완결성 검사)+§7.4(드래그 리사이즈) 묶음 → 3)
  §6.1(체크한 커밋 유지)+§7.3(파일명 검색) 묶음. 이번 작업이 이 중 하나라면
