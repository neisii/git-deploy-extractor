...
- 실사용 중 나온 개선 아이디어 5건 중 1건(사용자정의 Export 경로, §7.1)은
  2026-08-07에 구현 완료되어 REQUIREDMENT.md REQ-012/DR-013으로 정식 편입됐습니다
  (RISK_ISSUES.md 결정 이력 #24 참고). 나머지 4건(파일명 커밋 검색, 체크한 커밋
  유지, 의존성 완결성 검사, 드래그 리사이즈)은 이미 요구사항 수준까지 구체화만
  돼 있고 미구현 상태입니다 — RISK_ISSUES.md §6/§7에 배경·트레이드오프·확정
  설계·TO-BE 와이어프레임까지 다 있습니다. 이번 작업이 이 중 하나라면 처음부터
  다시 요구사항을 도출하지 말고 거기서부터 시작하세요(§0.2와 같은 이유 — 이미
  있는 걸 임의로 다시 만들지 않는다). REQUIREDMENT.md/DETAILED_DESIGN.md 등 정식
  문서로의 편입은 아직 안 돼 있으니, 구현하면서 편입하면 됩니다.
- 남은 4건의 권장 구현 순서(근거는 RISK_ISSUES.md §7 도입부 참고): 1) §7.2(의존성
  완결성 검사)+§7.4(드래그 리사이즈) 묶음 → 2) §6.1(체크한 커밋 유지)+§7.3(파일명
  검색) 묶음. §7.1이 끝나 있으므로 §7.2의 좌측 패널 설계("Server Path 열 삭제")를
  다시 고민할 필요는 없습니다(이미 §7.1에서 처리됨). 이번 작업이 이 중 하나라면
  이 순서를 따르는 걸 권장합니다(단, 기술적 강제는 아니고 사용자 지시가
  우선합니다).

먼저 이 순서로 읽어주세요 (짐작하지 말고 실제로 읽어야 합니다):

1. REQUIREDMENT.md — 요구사항 원문. REQ-001~012, DR-001~013이 전부 확정 사항.
