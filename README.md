...
8. **사용자가 마지막에 개별 파일을 제외할 수 있다** — Preview로 계산된 목록에서
   체크박스를 해제한 파일은 최종 추출/`extract-list.txt`의 "배포 대상 파일"
   목록에서 빠진다.
...
11. **파일 수동 추가(선택적, HEAD 기준 별도 경로)**: 10번과 마찬가지로 커밋
    ...
    직접 검색해 배포 대상에 추가할 수 있다. Java 파일로 제한되지 않으며,
    ...
    보완하기 위함이다. 자동완성 검색으로 후보를 고르는 방식만 지원한다(오타로
    잘못된 경로가 추가되는 것을 방지). 추가된 파일도 원본 바이트 그대로
    복사되고(5번과 동일) Mapping Rule도 동일하게 적용된다(7번과 동일).
