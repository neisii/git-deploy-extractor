...
export function PreviewSummary(): React.JSX.Element {
  ...
  if (phase === 'error') {
    return (
      <div className="panel deployment-preview-panel deployment-preview-panel--error fill">
        계산 실패: {analysisError}
      </div>
    )
  }
  ...
  if (phase === 'empty') {
    return <div className="panel deployment-preview-panel fill">커밋을 선택하세요</div>
  }

  if (phase === 'loading') {
    return <div className="panel deployment-preview-panel fill">계산 중...</div>
  }

  if (phase === 'stale' || !summary) {
    return (
      <div className="panel deployment-preview-panel fill">
