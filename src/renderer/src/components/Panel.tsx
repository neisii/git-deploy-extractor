...
// `min-height: 0` + `overflow: auto`를 가져서, 넘치는 내용이 있어도
// 헤더는 고정된 채 본문 안에서만 스크롤된다.
//
// 기존 `.panel`(main.css) 클래스는 지금도 여러 화면이 헤더/본문 구분 없이
// 단일 `<div>`로 쓰고 있어(예: DeploymentPreviewPanel), 그 시맨틱을 바꾸면
...
export function PanelBody({ children, className }: SlotProps): React.JSX.Element {
  return (
    <div className={['panel-frame__body', className].filter(Boolean).join(' ')}>{children}</div>
  )
}
