...
**실현 완료 (2026-08-07), 2026-08-12 재작성, 2026-08-20 재작성** — §7.1~7.4가 전부 구현되면서 아래 그림이 실제 화면 모습이 됐다. 이 섹션 자체는 구현 대상이 아니라 그 결과를 미리 그려본 설계 문서였으므로 "구현 완료" 대신 "실현 완료"로 표기한다. REQUIREDMENT.md §8의 AS-IS 원본과는 달리 이 섹션은 구현이 바뀔 때마다 같이 갱신하는 게 원칙이다 — 아래는 REQ-019/020(제외 패턴, 선택 카운터)과 전체 선택 체크박스 위치 이동까지 반영한 최신 스냅샷이다.

```
┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│ deep-backend (shallow-backend) / main   D:\workspace\shallow-backend        [Browse...] [Reload]   [v0.3.0]  │  ← REQ-018: 별도 TitleBar 행 없음,
...
├────────────────────────────────────────────────────┬─┬─────────────────────────────────────────────────────┤
│ 포함된 파일 (선택 3개/전체 3개(필터 전 전체 5개))    │⋮│ 누락된 의존성 (선택 0개/전체 55개(필터 전 전체 55개))│  ← REQ-020: 세 숫자 카운터
│                                     Filter: All ▼   │⋮│                                                       │     (우측 50개 초과 시
│ 검색(파일명): [                    ]                │⋮│ 검색(파일명): [                    ]                  │      "전체 N개"만 붉은 글씨)
│ 제외 패턴: [ *.png       ] [+추가]  [*.png][*.md]   │⋮│                                                       │  ← REQ-019: 좌측만(칩=이력,
│──────────────────────────────────────────────────── │⋮│──────────────────────────────────────────────────── │     활성만 강조 표시)
│ ☑ .../GuaranteeListController.java                  │⋮│ ☑ .../GuaranteeListExternalRepository.java  (구현체) │  ← 컬럼 헤더 행 체크박스가
│ ☑ .../static/js/guarantee/list.js                   │⋮│ ☑ .../GuaranteeListQueryRepository.java  (인터페이스)│     Local Path 왼쪽,
│ ☑ .../templates/guarantee/list.html                 │⋮│                                                       │     각 행과 동일 x 위치
│  (icon.png, logo.png 등은 제외 패턴에 걸려 안 보임)   │⋮│                                                       │     (2026-08-20 위치 이동,
│  ※ Local Path만 표시, Server Path 열 삭제(§7.1-5)   │⋮│  ※ 커밋 이력엔 없지만 정적 분석상 의존하는 클래스     │     우측도 체크박스로 통일
│  ※ 내용 길면 가로 스크롤(양쪽 다, 300개 초과 시도    │⋮│  ※ 전체선택은 이 검색·Filter·제외패턴에 걸린 행만    │     — "전체 추가" 버튼 폐지)
