...
# 7. 차기 기능 요구사항 초안 (§7.1 구현 완료, §7.2~7.5 미구현)

§6과 달리 아래는 선택지 논의까지 끝내고 **최종 결정까지 마친** 신규 기능 초안이다. §7.1은 REQUIREDMENT.md REQ-012/DR-013으로 정식 편입되고 구현까지 완료됐다(결정 이력 #24 참고) — 나머지(§7.2~7.5)는 아직 초안 상태이며, 실제 구현 세션에서 이 내용을 REQ/DR로 정식 편입하고 DETAILED_DESIGN.md 수준까지 설계를 이어가면 된다.
