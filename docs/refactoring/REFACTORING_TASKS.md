...
| M-42 | ~~조회 조건 두 그룹(경계 조절)의 세부(가정)~~ — **결정(RT-49, 2026-09-23, 사용자 확인): 가정대로 전부 채택** — (a) 기본 비율 50:50·최소 폭 320/330px (b) 가로 스크롤 유지(세로로 안 쌓음) (c) 경계 키보드 조절 없음 (d) 힌트 툴팁은 필드 아래(포커스 유지 중 아래 커밋 영역을 일시적으로 가릴 수 있음) | (결정됨) |
